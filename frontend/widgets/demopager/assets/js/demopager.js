    ...
    bindEvent: function (pageSizeParam, pageParam, url) {
        ...
        $('input[name="' + pageParam + '"]').on('keydown', function (evt) {
            if (evt.which == 13) {
                ...
                // var pattern = new RegExp("\(&" + pageParam + "=\)\\d+");
                // var pattern = new RegExp("\(&page=\)\\d+", "gi");
                console.log(pattern);
                console.log("$1"+targetPage);
                // var newUrl = url.replace(pattern, "$1"+targetPage);
                // var newUrl = url.replace(pattern, targetPage);

                var pattern = new RegExp("/([?|&]" + pageParam + "=)[^\&]+/");
                newUrl = url.replace( pattern , '$1' + targetPage);
                console.log(newUrl);
